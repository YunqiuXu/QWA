...
class KG_Manipulation(torch.nn.Module):
    ...
    def embed(self, input_words):
        word_embeddings, mask = self.word_embedding(input_words)   
        word_embeddings = self.word_embedding_prj(word_embeddings)
        word_embeddings = word_embeddings * mask.unsqueeze(-1)   
        return word_embeddings, mask

    def encode_text(self, input_word_ids):
        embeddings, mask = self.embed(input_word_ids)   
        squared_mask = torch.bmm(mask.unsqueeze(-1), mask.unsqueeze(1))   
        encoding_sequence = embeddings
        for i in range(self.encoder_layers):
            encoding_sequence = self.encoder[i](encoding_sequence, squared_mask, i * (self.encoder_conv_num + 2) + 1, self.encoder_layers)   
    ...
    def encode_text_for_pretraining_tasks(self, input_word_ids):
        ...
        encoding_sequence = embeddings
        for i in range(self.encoder_layers):
            encoding_sequence = self.encoder_for_pretraining_tasks[i](encoding_sequence, squared_mask, i * (self.encoder_conv_num + 2) + 1, self.encoder_layers)   
        return encoding_sequence, mask

    def get_graph_node_representations(self, node_names_word_ids):
        node_name_embeddings, _mask = self.embed(node_names_word_ids)   
        _mask = torch.sum(_mask, -1)   
        node_name_embeddings = torch.sum(node_name_embeddings, 1)   
        tmp = torch.eq(_mask, 0).float()
        if node_name_embeddings.is_cuda:
            tmp = tmp.cuda()
        _mask = _mask + tmp
        node_name_embeddings = node_name_embeddings / _mask.unsqueeze(-1)
        node_name_embeddings = node_name_embeddings.unsqueeze(0)   
        node_ids = np.arange(self.node_vocab_size)   
        node_ids = to_pt(node_ids, enable_cuda=node_names_word_ids.is_cuda, type='long').unsqueeze(0)   
        node_embeddings, _ = self.node_embedding(node_ids)   
        node_embeddings = torch.cat([node_name_embeddings, node_embeddings], dim=-1)   

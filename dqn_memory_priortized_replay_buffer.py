...
class PrioritizedReplayMemory(object):
    ...
    def _get_single_transition(self, idx, n):
        ...
        return (obs, prev_action, candidate, 
                tasks, 
                chosen_indices, graph_triplets, 
                reward + graph_reward + count_reward, 
                next_obs, next_prev_action, next_candidate, next_graph_triplets)

    def _encode_sample(self, idxes, ns):
        actual_indices, actual_ns = [], []
        obs, prev_action, candidate, tasks, chosen_indices, graph_triplets, reward, next_obs, next_prev_action, next_candidate, next_graph_triplets = [], [], [], [], [], [], [], [], [], [], []
        for i, n in zip(idxes, ns):
            ...
            graph_triplets.append(t[5])
            reward.append(t[6])
            next_obs.append(t[7])
            next_prev_action.append(t[8])
            next_candidate.append(t[9])
            next_graph_triplets.append(t[10])

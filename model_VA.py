...
class Action_Validator(torch.nn.Module):
    model_name = 'action_validator'

    def __init__(self, config, word_vocab, node_vocab, relation_vocab):
        super(Action_Validator, self).__init__()
        self.config = config
    ...
    def read_config(self):
        ...
        assert not self.enable_recurrent_memory, "Disable rnn!"
    ...
    def _def_layers(self):
        ...
        self.scorer_linear_1 = linear_function(self.block_hidden_dim * 2, self.block_hidden_dim)
        self.scorer_linear_2 = linear_function(self.block_hidden_dim, 2)
    ...
    def encode_text(self, input_word_ids):
        ...
        for i in range(self.encoder_layers):
            encoding_sequence = self.encoder[i](encoding_sequence, 
                                                squared_mask, i * (self.encoder_conv_num + 2) + 1, 
    ...
    def encode_candidates(self, input_candidate_word_ids):
        ...
        return candidate_representations, cand_mask

    def score_task_action_pair(self, input_task_ids, input_action_ids):
        _, num_tasks, task_candidate_len = input_task_ids.size(0), input_task_ids.size(1),input_task_ids.size(2)
        assert num_tasks == 1, "num_tasks should always be 1, got {}".format(num_tasks)
        task_representations, task_mask = self.encode_candidates(input_task_ids)
        task_representations = task_representations.squeeze(1) 
        _, num_actions, action_candidate_len = input_action_ids.size(0), input_action_ids.size(1), input_action_ids.size(2)
        assert num_actions == 1, "num_actions should always be 1, got {}".format(num_actions)
        action_representations, action_mask = self.encode_candidates(input_action_ids) 
        action_representations = action_representations.squeeze(1) 
        output = self.scorer_linear_1(torch.cat([task_representations, action_representations],-1))
        output = torch.relu(output)
        output = self.scorer_linear_2(output) # [batch, 2]
        return output

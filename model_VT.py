...
class Task_Generator(torch.nn.Module):
    model_name = 'task_generator'
    ...
    def _def_layers(self):
        ...
        self.task_scorer_linear_1 = linear_function(self.block_hidden_dim * 2, self.block_hidden_dim)
        self.task_scorer_linear_2 = linear_function(self.block_hidden_dim, 2)
    ...
    def score_graph_task_pair(self, input_task_candidate_ids, h_go, node_mask):
        batch_size, num_task_candidate, task_candidate_len = input_task_candidate_ids.size(0), input_task_candidate_ids.size(1), input_task_candidate_ids.size(2)
        assert num_task_candidate == 1, "num_task_candidate should always be 1, got {}".format(num_task_candidate)
        task_candidate_representations, task_cand_mask = self.encode_candidates(input_task_candidate_ids)
        ...
        task_candidate_representations = task_candidate_representations.squeeze(1)
        task_output = self.task_scorer_linear_1(torch.cat([task_candidate_representations, graph_representations],-1))
        task_output = torch.relu(task_output)
        task_output = task_output * task_cand_mask # [batch, hid]
        task_output = self.task_scorer_linear_2(task_output) # [batch, 2]
        task_output = task_output * task_cand_mask
        return task_output, task_cand_mask
